...
//	2 バイトコードの表示、c:キャラゼネコード
int sjis_disp_char_x1(int x, int y, int c)
{
	int i, j, cg, bit /*, color*/;
	
//	x2 = x + (12-1);
//	y2 = y + (16-1);
	lcd_area_square(x, y, 12, 16);
	lcd_write_cmd(MemoryWrite);
...
//	LCD_DC_H;                           // data
	bit = 0x80;
	for(i=0; i<12*16; i++){				// 16 行を順番に処理する
		j = (i-24)/8;
		cg = KanjiFont12b[c][j];		// キャラゼネ１行
		if((cg & bit) && (i>=12*2 && i<12*14)){
			lcd_write_color(text_color);
//			while(SPI2STATbits.SPITBE==0)
//				;
//			SPI2BUF = text_color>>8;
//			while(SPI2STATbits.SPITBE==0)
//				;
//			SPI2BUF = text_color;
		}
...
//	LCD_DC_H;                           // data
	...
	for(i=0; i<12*16; i++){				// 16 行を順番に処理する
		j = (i-24)/8;
		cg = KanjiFont12b[c][17-j];		// キャラゼネ１行
		if((cg & bit) && (i>=12*2 && i<12*14)){
			lcd_write_color(text_color);
//			while(SPI2STATbits.SPITBE==0)
//				;
...
{
	return lcd_area_set(x,y, x+w-1, y+h-1);
}
/********************************************************************************/
/*    lcd_clear                               */
...
{
	lcd_clearA(x, y, x+w-1, y+h-1, color);
}
...
{
	lcd_clear1_square(0,0, 320,240, color);
}
...
//	digitalWrite(LCD_RD, LOW);
	Port_RD->PIO_CODR = Mask_RD;		// RD LOW
...
//	digitalWrite(LCD_RD, HIGH);
	Port_RD->PIO_SODR = Mask_RD;		// RD HIGH
...
{
	lcd_read_mem(x, y, x+w-1, y+h-1, buf);
}
/********************************************************************************/
/*    lcd_write_data                              */
/********************************************************************************/
...
void circle1(int x0, int y0, int r, int color)
{
	...
	f=-2*r+3;
	while(x>=y){
		lcd_clear_square(x0-x, y0-y, 1,1, color);
		lcd_clear_square(x0-x, y0+y, 1,1, color);
		lcd_clear_square(x0+x, y0-y, 1,1, color);
		lcd_clear_square(x0+x, y0+y, 1,1, color);
		lcd_clear_square(x0-y, y0-x, 1,1, color);
		lcd_clear_square(x0-y, y0+x, 1,1, color);
		lcd_clear_square(x0+y, y0-x, 1,1, color);
		lcd_clear_square(x0+y, y0+x, 1,1, color);
		if(f>=0){
...
void circlefill(int x0, int y0, int r, int c)
{
	...
	f=-2*r+3;
	while(x>=y){
		lcd_clear_square(x0-x, y0-y, 2*x+1, 1, c);
		lcd_clear_square(x0-x, y0+y, 2*x+1, 1, c);
		lcd_clear_square(x0-y, y0-x, 2*y+1, 1, c);
		lcd_clear_square(x0-y, y0+x, 2*y+1, 1, c);
		if(f>=0){
			x--;
			f-=x*4;
...
const uint8_t KanjiFont12b[][18]={
{0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0},	// dummy data				code
{0x04,0x00,0x40,0x27,0xc2,0x40,0x24,0x0f,0xfe,0x24,0x84,0x44,0x0c,0x00,0x0c,0x07,0x07,0x80,},   // 歩	1
{0x00,0x07,0xfc,0x04,0x0f,0xfe,0x15,0x02,0x48,0xc4,0x43,0xfa,0x20,0x83,0xf8,0x20,0x83,0xf8,},   // 香	2
{0x01,0x02,0x10,0x27,0xef,0x10,0x21,0x02,0xfe,0x61,0x06,0x10,0xb7,0xea,0x10,0x21,0x02,0xfe,},   // 桂	3
{0x00,0x03,0x3e,0x4a,0x28,0x3e,0xfa,0x22,0x22,0xfb,0xe2,0x28,0xaa,0xaa,0xaa,0x22,0x4f,0xb2,},   // 銀	4
{0x00,0x00,0xe0,0x11,0x02,0x08,0x5f,0x48,0x42,0x04,0x07,0xfc,0x04,0x04,0x44,0x24,0x8f,0xfe,},   // 金	5
{0x00,0x01,0xf8,0x21,0x07,0xfc,0xc4,0x47,0xfc,0x44,0x44,0x44,0x7f,0xc4,0x04,0x80,0x48,0x1c,},   // 角	6
{0x00,0x07,0xf2,0x01,0x41,0x58,0xe5,0x42,0x52,0x24,0x4f,0xf8,0x25,0x42,0x52,0x44,0x84,0x46,},   // 飛	7
{0x00,0x0f,0xfe,0x04,0x00,0x40,0x04,0x00,0x40,0x7f,0xc0,0x40,0x04,0x40,0x42,0x04,0x0f,0xfe,},   // 玉	8
{0x00,0x00,0x80,0x08,0x00,0x80,0x04,0x00,0x78,0x0c,0x01,0x00,0x20,0x02,0x00,0x10,0x00,0xf8,},   // と	9
{0x04,0x00,0x40,0xff,0xe0,0x40,0x0d,0x03,0x48,0xc4,0x40,0x42,0x3f,0x82,0x08,0x20,0x83,0xf8,},   // 杏	10
{0x04,0x00,0x40,0x7f,0xc0,0x40,0x04,0x0f,0xfe,0x04,0x00,0x40,0x7f,0xc0,0x40,0x04,0x0f,0xfe,},   // 圭	11
{0x00,0x00,0xe0,0x11,0x02,0x08,0x40,0x4b,0xfa,0x04,0x00,0x40,0x3f,0x80,0x40,0x04,0x0f,0xfe,},   // 全	12
{0x00,0x07,0xfc,0x44,0x07,0xf8,0x44,0x07,0xf8,0x44,0x07,0xfe,0x00,0x25,0x52,0x95,0x20,0x0c,},   // 馬	13
{0x04,0x00,0x40,0xff,0xe1,0x10,0x11,0x0f,0xfe,0x24,0x83,0xf8,0x24,0x83,0xfa,0x04,0x20,0x7e,},   // 竜	14
};
