...
#define NO_TIMEOUT	1234567
extern int lcd_x;                  // 0・ｽE・ｽ`25
extern int lcd_y;                  // 0・ｽE・ｽ`14
...
uint32_t USBHost::ctrlReq(uint32_t addr, uint32_t ep, uint8_t bmReqType, uint8_t bRequest, uint8_t wValLo, uint8_t wValHi, uint16_t wInd, uint16_t total, uint32_t nbytes, uint8_t* dataptr, USBReadParser *p)
{
	...
	// Dispatch packet
	while((rcode = dispatchPkt(tokSETUP, pep->hostPipeNum, nak_limit))==NO_TIMEOUT)	//AAAAA
		;
	if (rcode)
	{
	...
	if (dataptr != 0)
	{
		if (direction)
		{
			...
			while (left)
			{
				...
				while((rcode = InTransfer(pep, nak_limit, &read, dataptr))==NO_TIMEOUT)		//AAAAA
					;
				if (rcode)
					return rcode;
	...
	// Status stage
	while((rcode = dispatchPkt((direction) ? tokOUTHS : tokINHS, pep->hostPipeNum, nak_limit))==NO_TIMEOUT)	//AAAAA
		;
	return rcode;
}
 ...
 uint32_t USBHost::inTransfer(uint32_t addr, uint32_t ep, uint32_t *nbytesptr, uint8_t* data)
{
	...
	rcode = InTransfer(pep, nak_limit, nbytesptr, data);	//AAAAA
	return rcode;
}
...
//	uint32_t nbytes = *nbytesptr;
	...
	static uint8_t *data_save = 0;		//AAAAA
	static uint32_t nbytesptr_save;
	static uint32_t nbytes;

//	*nbytesptr = 0;		//AAAAA
	
	if(data_save==0){	//AAAAA
		nbytesptr_save = 0;
		nbytes = *nbytesptr;
		data_save = data;
	}
...
//	{
		...
		if((rcode = dispatchPkt(tokIN, pep->hostPipeNum, nak_limit))==NO_TIMEOUT)	//AAAAA
			return NO_TIMEOUT;
        if (rcode)
		{
			if (rcode == 1)
        ...
        data_save += UHD_Pipe_Read(pep->hostPipeNum, pktsize, data_save);

		// Add this packet's byte count to total transfer length
//		*nbytesptr += pktsize;		//AAAAA
		nbytesptr_save += pktsize;	//AAAAA

        // The transfer is complete under two conditions:
        // 1. The device sent a short packet (L.T. maxPacketSize)
        // 2. 'nbytes' have been transferred.
//		if ((pktsize < maxpktsize) || (*nbytesptr >= nbytes))		//AAAAA
		if ((pktsize < maxpktsize) || (nbytesptr_save >= nbytes))
		{
			data_save = 0;	//AAAAA
			*nbytesptr = nbytesptr_save;
            return 0;
        }
//	}
	return NO_TIMEOUT;
...
uint32_t USBHost::OutTransfer(EpInfo *pep, uint32_t nak_limit, uint32_t nbytes, uint8_t *data)
{
	...
	while (bytes_left)
	{
		...
		while((rcode = dispatchPkt(tokOUT, pep->hostPipeNum, nak_limit))==NO_TIMEOUT)	//AAAAA
			;
		if (rcode)
		{
...
//	while ((timeout > millis()) && (UHD_GetVBUSState() == UHD_STATE_CONNECTED))
	if((UHD_GetVBUSState() == UHD_STATE_CONNECTED))		//AAAAA
	{
...
//static int cnt;
//lcd_x=lcd_y=0;lcd_printf("%x %d \n", usb_task_state, ++cnt);//AAAAA
